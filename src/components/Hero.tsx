...
export default function Hero({ onOpenModal }: { onOpenModal: () => void }) {
  return (
    <section id="desk" className="pt-24 pb-16 sm:pb-20 lg:pt-32 lg:pb-32 overflow-hidden relative bg-white">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 grid lg:grid-cols-2 gap-12 lg:gap-20 items-center relative z-10">
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-2 sm:flex sm:flex-wrap sm:gap-3 mb-10 sm:mb-12">
            {['后台事务', '一站服务', '可视可控'].map((point) => (
              <div key={point} className="flex min-w-0 items-center justify-center gap-1.5 sm:gap-2 whitespace-nowrap text-xs sm:text-sm font-medium text-slate-600 bg-slate-50 px-2 sm:px-4 py-2.5 rounded-full border border-slate-100">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            ...
            <a 
              href="#path"
              className="inline-flex items-center justify-center h-14 px-8 rounded-full bg-white text-slate-600 font-medium text-base sm:text-lg border border-slate-200 hover:border-slate-300 hover:text-slate-900 transition-all gap-2"
            >
              注册新的公司
              <ArrowRight size={20} />
            </a>
        ...
        <motion.div
           ...
           className="relative mx-auto w-full max-w-sm sm:max-w-md lg:max-w-none pattern-bg rounded-[32px] sm:rounded-[40px] p-4 sm:p-8 border border-slate-100 flex items-center justify-center"
        >
           <PhoneMockup />
        </motion.div>
      </div>
